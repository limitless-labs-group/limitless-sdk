...
- **WebSocket HMAC auth on reconnect**: `WebSocketClient` now hands python-socketio a header *callable* instead of a pre-built dict, so a fresh `lmts-timestamp` and `lmts-signature` are computed before every connection attempt. Previously the signature was computed once in `connect()` and python-socketio replayed the same headers on each automatic reconnect, so any reconnect after the server's timestamp window expired failed authentication. API-key and SDK-tracking headers go through the same callable; their values are unchanged.
