...
@pytest.mark.asyncio
async def test_create_order_threads_stp_policy_top_level(monkeypatch):
    http_client = Mock()
    http_client.require_auth = Mock()
    http_client.post = AsyncMock(return_value=_order_response_payload())
    service = DelegatedOrderService(http_client)
    ...
    await service.create_order(
        ...
        price=0.55,
        size=5.0,
        stp_policy="cancel_taker",
    )

    _, payload = http_client.post.await_args.args
    # top-level, never inside the signed order struct
    assert payload["stpPolicy"] == "cancel_taker"
    assert "stpPolicy" not in payload["order"]


@pytest.mark.asyncio
async def test_create_order_omits_stp_policy_when_unset(monkeypatch):
    http_client = Mock()
    http_client.require_auth = Mock()
    http_client.post = AsyncMock(return_value=_order_response_payload())
    service = DelegatedOrderService(http_client)
    ...
    _, payload = http_client.post.await_args.args
    assert "stpPolicy" not in payload
